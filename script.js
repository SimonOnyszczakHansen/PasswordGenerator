document.addEventListener("DOMContentLoaded", function () {
  ...
  const capitalizeFirstLetter = document.getElementById(
    "capitalizeFirstLettersCheckbox"
  );
  ...
  const languages = {
    ...
    de: {
      ...
      strong: "Stark",
      minInterestsMessage:
        "Bitte fügen Sie mindestens {minInterests} Interessen hinzu, um ein Passwort zu generieren.",
    },
    ar: {
      ...
      strong: "قوي",
      minInterestsMessage:
        "يرجى إضافة {minInterests} اهتمامات على الأقل لتوليد كلمة مرور.",
    },
  };
  ...
  function applyTranslation(lang) {
    ...
    document.getElementById("useSpecialCharactersLabel").textContent =
      selectedLang.useSpecialCharacters;
    document.getElementById("capitalizeFirstLettersLabel").textContent =
      selectedLang.capitalizeFirstLetter;
  ...
  function checkMinimumInterests() {
    ...
    if (totalPasswordLength <= serviceNameLength) {
      minInterestsFeedback.textContent =
        "Password length must be greater than the length of characters from service name.";
  ...
  function generateBasePassword(tags, charactersValue, maxLength) {
    ...
    let specialCharacterUsed = false; // Flag to track if a special character has been used
    ...
    nonNumberTags.forEach((tag) => {
      ...
      // Replace the first possible letter with a special character if enabled and not already done
      if (useSpecial && !specialCharacterUsed) {
        extractedPart = extractedPart.replace(/o|l|g|s|e/i, (char) => {
          specialCharacterUsed = true; // Mark that a special character has been used
    ...
    numbers.forEach((number) => {
      ...
      basePassword =
        basePassword.slice(0, randomIndex) +
        number +
        basePassword.slice(randomIndex);
    });
